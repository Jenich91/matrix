...
matrix_t sfleta_sum_matrix(matrix_t *A, matrix_t *B) {
    ...
    if (A->rows != B->rows || A->columns != B->columns) {
        ...
        resultMatrix = sfleta_create_matrix(A->rows, A->columns);
...
matrix_t sfleta_mult_number(matrix_t *A, double number) {
    matrix_t resultMatrix = sfleta_create_matrix(A->rows, A->columns);
...
matrix_t sfleta_mult_matrix(matrix_t *A, matrix_t *B) {
    ...
    if (A->columns != B->rows) {
        ...
        resultMatrix = sfleta_create_matrix(A->rows, B->columns);
...
matrix_t sfleta_transpose(matrix_t *A) {
    ...
    if (A->matrix_type != INCORRECT_MATRIX && A->columns > 0 && A->rows > 0) {
        resultMatrix = sfleta_create_matrix(A->columns, A->rows);
    ...
    if (A->rows == A->columns && A->matrix_type != INCORRECT_MATRIX) {
        ...
        if (size == 1) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                determinant = determinant + sign * A->matrix[i][0] * sfleta_determinant(&newMatrix);
                sign =- sign;
                sfleta_remove_matrix(&newMatrix);
            }
        }
    }
...
matrix_t getSubMatrix(matrix_t *src, int rowForDelete, int columnForDelete) {
    matrix_t newMatrix = sfleta_create_matrix(src->rows-1, src->columns-1);
    ...
    if ((A->rows == A->columns) && (A->rows > 1) && (A->matrix_type != INCORRECT_MATRIX)) {
        ...
        for (size_t i = 0; i < size; i++)  {
            for (size_t j = 0; j < size; j++) {
                ...
                resultMatrix.matrix[i][j] = pow(-1, ((j+1)+(i+1))) * sfleta_determinant(&subMatrix);
                sfleta_remove_matrix(&subMatrix);
            }
        }
        resultMatrix.matrix_type = GetMatrixType(&resultMatrix);
    } else {
...
matrix_t sfleta_inverse_matrix(matrix_t *A) {
    ...
    if (A->rows == 1 && A->columns == 1) {
        if (isEquale(A->matrix[0][0], 0)) {
            ...
            resultMatrix = sfleta_create_matrix(1, 1);
            resultMatrix.matrix[0][0] = 1/A->matrix[0][0];
            resultMatrix.matrix_type = GetMatrixType(&resultMatrix);
        }
    } else if (A->matrix_type != INCORRECT_MATRIX) {
        double det = sfleta_determinant(A);

        if (isEquale(det, 0) || isnan(det)) {
            resultMatrix.matrix_type = INCORRECT_MATRIX;
        } else {
            matrix_t C = sfleta_calc_complements(A);
            matrix_t T = sfleta_transpose(&C);
            resultMatrix = sfleta_mult_number(&T, 1/det);

            resultMatrix.matrix_type = GetMatrixType(&resultMatrix);
            sfleta_remove_matrix(&C);
            sfleta_remove_matrix(&T);
        }
    } else {
        resultMatrix.matrix_type = INCORRECT_MATRIX;
    }
